...
def main(
    ...
    """
    Évalue un modèle à partir de fichiers de prédictions et d'étiquettes.
    
    Args:
        predictions_path: Chemin vers le fichier des prédictions
        labels_path: Chemin vers le fichier des vraies étiquettes
        metrics_output: Chemin de sortie pour les métriques
        task_type: Type de tâche ('classification' ou 'regression')
    """
    logger.info("Chargement des données...")
    
    # Charger les prédictions et les vraies valeurs
    predictions_df = pd.read_csv(predictions_path)
    labels_df = pd.read_csv(labels_path)
    
    y_true = labels_df.values.ravel()
    y_pred = predictions_df["predictions"].values if "predictions" in predictions_df.columns else predictions_df.values.ravel()
    
    # Probabilités si disponibles
    y_proba = None
    if "probabilities" in predictions_df.columns or "proba_0" in predictions_df.columns:
    ...
    typer.echo(f"Évaluation terminée. Métriques: {metrics_output}")
